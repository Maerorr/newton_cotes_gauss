use eframe::{
    egui::{self, plot::{Points, Plot, Values, Value, Line, VLine, PlotUi, PlotImage}, Layout, Image},
    epi::{App}, run_native, epaint::{Color32, vec2, TextureId},
};
...
impl App for AppState {
    fn update(&mut self, ctx: &eframe::egui::Context, frame: &eframe::epi::Frame) {
                    ...
                    .show(ctx, |ui| {
                        ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
                            ...
                            if ui.button("Calculate").clicked() {
                                ...
                                for i in 0..1000 {
                                    let x = self.left - 5.
                                        + i as f64 * ((self.left) - (self.left - 5.))
                                            / 1000.;
                                ...
                                for i in 0..1000 {
                                    let x = self.right
                                        + i as f64 * ((self.right + 5.) - (self.right))
                                            / 1000.;
                                    vec.push(Value::new(x, function_value(x, self.function)));
                                }
                                self.plot_right = vec;
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let f_plot_left = Line::new(f_left).name("plot").color(Color32::from_rgb(100, 150, 250));
            let f_plot_mid = Line::new(f_mid).name("plot").fill(0.).color(Color32::from_rgb(100, 150, 250));
            let f_plot_right = Line::new(f_right).name("plot").color(Color32::from_rgb(100, 150, 250));
            let mut plot = Plot::new("plot").legend(egui::widgets::plot::Legend::default());
            let left = VLine::new(self.left).color(Color32::from_rgb(10, 77, 154)).name("Integral Range");
            let right = VLine::new(self.right).color(Color32::from_rgb(10, 77, 154)).name("Integral Range");
